...
public class LinkedList<E extends Comparable<E>> {
	...
	private void removeAll(Node temp, E el) {
		if (temp.next == null)
			return;
		else if (el.equals(temp.data)) {
			first = first.next;
			n--;
			removeAll(first, el);
		} else {
			if (el.equals(temp.next.data)) {
				temp.next = temp.next.next;
				n--;
				removeAll(temp, el);
			} else {
				removeAll(temp.next, el);
			}
		}
	}
	...
	private void duplicateAll(Node temp, E el) {
		if (temp == null)
			return;
